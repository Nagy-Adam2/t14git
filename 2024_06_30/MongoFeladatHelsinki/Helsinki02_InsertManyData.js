...
function ObjektumFeltolto(feltoltendoElem) {
    ...
    for (i = 0; i < feltoltendoElem.length; i++) {
...
async function ObjektumFeltoltesMongoDB() {
    try {
        const client = await MongoClient.connect(url);
        const db = client.db("Helsinki");

        const ujAdatok = ObjektumFeltolto(eredmenyek)

        const muveletek = await db.collection("HelsinkiAdatokObjektumban").insertMany(ujAdatok);
